...
async function createSiteSpool(payload, options = {}) {
  ...
  const candidateTemplatePaths = [
    path.join(rootPath, templateFolderName),
    path.join(path.dirname(rootPath), templateFolderName),
  ];

  let templatePath = null;
  for (const candidatePath of candidateTemplatePaths) {
    const templateStat = await fs.stat(candidatePath).catch(() => null);
  ...
  if (!templatePath) {
    const attempted = candidateTemplatePaths.join(', ');
    throw new Error(`Template folder not found. Looked in: ${attempted}`);
  }

  const destinationBasePath = path.dirname(templatePath);
  const destinationPath = path.join(destinationBasePath, values.siteFolderName);
